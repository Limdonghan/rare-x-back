...
public class StorageRequestService {
    ...
    public List<StorageRequestResponseDto> getPendingStorageRequests(Long userId) {
        List<StorageRequest> requests = storageRequestRepository
                .findByUserIdAndStatus(userId, StorageRequestStatus.PENDING);
    ...
    // 발송 처리
    @Transactional
    public StorageRequestResponseDto shipToWarehouse(Long userId, Long storageRequestId) {
        // 1. 보관 신청 조회
        ...
        if (!storageRequest.getUser().getUserId().equals(userId)) {
